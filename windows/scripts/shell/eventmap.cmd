...
:set_var
set get=bin\%adb%.exe shell input keyevent
set gettxt=bin\%adb%.exe shell input text
...
for /f "delims=" %%a in ('inifile.exe "eventmap.ini" [%event%] command') do %%a
...
for /f "delims=" %%b in ('inifile.exe "eventmap.ini" [%event%] number') do %%b
