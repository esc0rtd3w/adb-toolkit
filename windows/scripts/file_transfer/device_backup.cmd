...
echo.
echo.
echo 1) Root directory on device (BACKUP EVERYTHING) (/)
echo.
echo 2) Root directory on SD Card (BACKUP SD CARD) (/sdcard/)
echo.
...
:: Fix empty path
if [%androidpath%]==[] set androidpath=%dl_path%
...
if %destination%==default set destination=%dl_path%
...
::adb backup -apk -shared -all -f C:\adbtk\full-backup-%autoserial%.ab
